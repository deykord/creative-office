...
import { DoorOpen, Eye, MessageCircle, Phone, UserMinus, X } from 'lucide-react';
...
export const UserActionMenu: React.FC<Props> = ({ target, currentUser, presence, rooms, x, y, onClose, onChat, onCall, onInvite, onKick }) => {
  useEffect(() => {
    const close = () => onClose();
  ...
  const canKick = !self && Boolean(room && (currentUser.isAdmin || room.ownerUserId === currentUser.id));
