...
import { Crown, Gamepad2, MicOff, Presentation, Users, Video } from 'lucide-react';
...
const Avatar: React.FC<{ user: User; presence?: PresenceStatus; speaking?: boolean; size?: AvatarSize; voiceOnly?: boolean; onMenu: (user: User, event: React.MouseEvent) => void }> = ({ user, presence, speaking, size = 'large', voiceOnly, onMenu }) => (
  <span role="button" tabIndex={0} onClick={(event) => { event.stopPropagation(); onMenu(user, event); }} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); event.stopPropagation(); onMenu(user, event as unknown as React.MouseEvent); } }} className="relative shrink-0 cursor-pointer rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-300" title={`${user.name} · Open actions`} data-speaking={speaking ? 'true' : 'false'}>
    {user.avatarUrl ? <img src={user.avatarUrl} alt={user.name} className={`${size === 'compact' ? 'h-7 w-7 min-[420px]:h-8 min-[420px]:w-8 lg:h-9 lg:w-9' : size === 'small' ? 'h-8 w-8 min-[420px]:h-9 min-[420px]:w-9 lg:h-12 lg:w-12' : 'h-11 w-11 sm:h-12 sm:w-12 lg:h-[4.25rem] lg:w-[4.25rem]'} rounded-full object-cover bg-[#111216] transition-all duration-150 ${speaking ? 'opacity-100 ring-1 ring-cyan-300 shadow-[0_0_12px_rgba(103,232,249,.3)]' : 'opacity-60 ring-1 ring-white/[.08]'}`} /> : <span className={`${size === 'compact' ? 'h-7 w-7 text-[9px] min-[420px]:h-8 min-[420px]:w-8 lg:h-9 lg:w-9' : size === 'small' ? 'h-8 w-8 text-[10px] min-[420px]:h-9 min-[420px]:w-9 lg:h-12 lg:w-12 lg:text-sm' : 'h-11 w-11 text-sm sm:h-12 sm:w-12 lg:h-[4.25rem] lg:w-[4.25rem] lg:text-lg'} rounded-full bg-gradient-to-br from-zinc-700 to-zinc-900 flex items-center justify-center font-semibold text-zinc-100 transition-all duration-150 ${speaking ? 'opacity-100 ring-1 ring-cyan-300' : 'opacity-60 ring-1 ring-white/[.08]'}`}>{user.name.slice(0, 1).toUpperCase()}</span>}
    {presence?.isMuted && <span data-microphone-badge="muted" title="Microphone muted" className={`absolute left-1/2 flex -translate-x-1/2 items-center justify-center rounded-full border border-red-300/15 bg-[#2a2023]/90 text-red-300/65 shadow-[0_3px_10px_rgba(0,0,0,.28)] backdrop-blur-sm ${size === 'compact' ? '-bottom-1 h-3.5 w-3.5' : '-bottom-1.5 h-4 w-4'}`}><MicOff className={size === 'compact' ? 'h-2 w-2' : 'h-2.5 w-2.5'} /></span>}
    {!voiceOnly && presence?.isCameraOn && <span data-camera-badge="on" title="Camera on" className={`absolute -left-1 -bottom-1 flex items-center justify-center rounded-full border border-blue-200/15 bg-blue-400/75 text-slate-950 shadow-sm ${size === 'compact' ? 'h-3.5 w-3.5' : 'h-4 w-4'}`}><Video className={size === 'compact' ? 'h-2 w-2' : 'h-2.5 w-2.5'} /></span>}
  </span>
);
...
export const OfficeFloor: React.FC<OfficeFloorProps> = ({ rooms, users, presences, roomOccupancyMap, currentUser, currentRoomId, speakingUsers, onJoinRoom, onLeaveRoom, onKnock, onUserMenu, floor }) => {
  ...
  const occupantsFor = (room: Room) => (roomOccupancyMap[room.id] || []).map((id) => userById.get(id)).filter((user): user is User => Boolean(user && presences[user.id]?.status !== 'offline'));
  useEffect(() => {
            ...
            return (
              ...
              <button id={`btn-join-room-${room.id}`} type="button" onClick={isHere ? onLeaveRoom : () => onJoinRoom(room.id)} aria-label={isHere ? 'Leave Room' : joinLabel} data-room-type={room.type} className={`group relative w-full h-full rounded-[12px] sm:rounded-[17px] border overflow-hidden p-2 sm:p-3.5 text-left transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-300 ${isHere ? 'border-amber-300/70 bg-[#252117] ring-1 ring-amber-300/30' : 'border-white/[.09] bg-[#191a1f] hover:border-white/25 hover:bg-[#1d1e24] hover:-translate-y-0.5 hover:shadow-[0_20px_45px_rgba(0,0,0,.28)]'}`}>
                ...
                <span className={`absolute inset-0 pointer-events-none bg-gradient-to-br ${accent.split(' ')[0]} via-transparent to-transparent`} />
                  <span className="relative flex h-full flex-col"><span className="flex items-center justify-between gap-2"><span className="min-w-0 truncate text-xs font-semibold text-zinc-100 sm:text-base">{room.name}</span><Icon className="h-3.5 w-3.5 shrink-0 text-zinc-600 sm:h-4 sm:w-4" /></span>
