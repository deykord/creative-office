...
import { Activity, ArrowDown, ArrowUp, BarChart3, Building2, CalendarDays, Check, Clock3, Copy, DoorOpen, Download, Edit3, LayoutDashboard, LogIn, LogOut, Mail, MonitorUp, Plus, Radio, Save, ShieldCheck, Trash2, UserCog, Users, X } from 'lucide-react';
...
interface Analytics {
  ...
  members: { id: string; username: string; name: string; role: string; avatar_url: string; is_active: boolean; active_seconds: number; inactive_seconds: number; active_percent: number; session_count: number; first_seen: string | null; last_seen: string | null }[];
  ...
  events: { id: number; event_type: string; details: Record<string, unknown>; created_at: string; user_id: string; name: string; avatar_url: string; room_name: string | null }[];
}
...
const emptyUser = { name: '', username: '', role: 'Member', gender: 'male', password: '', isAdmin: false, canViewAnalytics: false, afkEnabled: true, afkAfterMinutes: 10, offlineAfterMinutes: 30, isActive: true, defaultFloorId: '' };
...
export const UserManagementModal: React.FC<Props> = ({ isOpen, onClose, users, rooms, floors, currentUser, onUsersChanged, onRoomsChanged, onFloorsChanged }) => {
  ...
  const [inviteForm, setInviteForm] = useState({ email: '', role: 'Member', gender: 'male', defaultFloorId: '' });
  const [inviteResult, setInviteResult] = useState<{ registrationUrl: string; mailtoUrl: string } | null>(null);
  ...
  useEffect(() => {
    ...
    setUserForm({ name: selectedUser.name, username: selectedUser.username, role: selectedUser.role, gender: selectedUser.gender || 'male', password: '', isAdmin: Boolean(selectedUser.isAdmin), canViewAnalytics: Boolean(selectedUser.canViewAnalytics), afkEnabled: selectedUser.afkEnabled !== false, afkAfterMinutes: selectedUser.afkAfterMinutes || 10, offlineAfterMinutes: selectedUser.offlineAfterMinutes || 30, isActive: selectedUser.isActive !== false, defaultFloorId: selectedUser.defaultFloorId || floors[0]?.id || '' });
  ...
  const saveUser = () => run(async () => {
    ...
    setUserForm({ ...emptyUser, defaultFloorId: floors[0]?.id || '' });
  ...
  const createInvitation = () => run(async () => {
    const data = await api('/api/admin/invitations', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...inviteForm, defaultFloorId: inviteForm.defaultFloorId || floors[0]?.id }) });
    setInviteResult(data.invitation); setInviteForm((current) => ({ ...current, email: '' }));
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-5 md:p-8">
            ...
            {tab === 'users' && <div className="grid xl:grid-cols-[1fr_420px] gap-6">
              ...
              <div className="bg-[#141418] border border-zinc-800 rounded-2xl overflow-hidden"><div className="px-5 py-4 border-b border-zinc-800 flex items-center justify-between"><div><h2 className="text-sm font-semibold">Workspace accounts</h2><p className="text-xs text-zinc-600 mt-1">Roles, access, and inactivity policies</p></div><button onClick={() => { setSelectedUserId(null); setUserForm({ ...emptyUser, defaultFloorId: floors[0]?.id || '' }); }} className="flex items-center gap-2 px-3 py-2 bg-amber-500 text-black text-xs font-bold rounded-lg"><Plus className="w-3.5 h-3.5" /> New</button></div><div className="divide-y divide-zinc-800">{users.map((user) => <button key={user.id} onClick={() => { setSelectedUserId(user.id); setUserForm({ name: user.name, username: user.username, role: user.role, gender: user.gender || 'male', password: '', isAdmin: Boolean(user.isAdmin), canViewAnalytics: Boolean(user.canViewAnalytics), afkEnabled: user.afkEnabled !== false, afkAfterMinutes: user.afkAfterMinutes || 10, offlineAfterMinutes: user.offlineAfterMinutes || 30, isActive: user.isActive !== false, defaultFloorId: user.defaultFloorId || floors[0]?.id || '' }); }} className={`w-full p-4 flex items-center gap-3 text-left hover:bg-zinc-900/70 ${selectedUserId === user.id ? 'bg-amber-500/5' : ''}`}><img src={user.avatarUrl} alt="" className="w-10 h-10 rounded-full" /><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="text-sm font-medium truncate">{user.name}</p>{user.isAdmin && <span className="text-[9px] uppercase font-bold text-amber-400 bg-amber-500/10 px-1.5 rounded">Admin</span>}{user.canViewAnalytics && !user.isAdmin && <span className="text-[9px] uppercase font-bold text-sky-300 bg-sky-400/10 px-1.5 rounded">Analytics</span>}{user.afkEnabled === false && <span className="text-[9px] uppercase font-bold text-violet-300 bg-violet-400/10 px-1.5 rounded">AFK off</span>}{user.isActive === false && <span className="text-[9px] uppercase font-bold text-red-400 bg-red-500/10 px-1.5 rounded">Disabled</span>}</div><p className="text-xs text-zinc-600 truncate">@{user.username} · {user.role} · {floors.find((floor) => floor.id === user.defaultFloorId)?.name || 'Unassigned'}</p></div><Edit3 className="w-4 h-4 text-zinc-700" /></button>)}</div></div>
...
function Overview({ analytics, users, range, customFrom, customTo, userId, onRangeChange, onCustomFromChange, onCustomToChange, onUserChange }: { analytics: Analytics | null; users: User[]; range: AnalyticsRange; customFrom: string; customTo: string; userId: string; onRangeChange: (range: AnalyticsRange) => void; onCustomFromChange: (value: string) => void; onCustomToChange: (value: string) => void; onUserChange: (id: string) => void }) {
  ...
  return <div className="space-y-6">
    ...
    <div className="bg-[#141418] border border-zinc-800 rounded-2xl overflow-hidden"><div className="p-5 border-b border-zinc-800"><h2 className="text-sm font-semibold">Activity audit log</h2><p className="text-xs text-zinc-600 mt-1">Sign-ins, media controls, and room movement</p></div><div className="max-h-[420px] overflow-y-auto divide-y divide-zinc-800/80">{analytics.events.map((event) => {
      ...
      return <div key={event.id} className="px-5 py-3.5 flex items-center gap-3 hover:bg-zinc-900/40"><div className="w-8 h-8 rounded-lg bg-zinc-900 flex items-center justify-center"><EventIcon className="w-3.5 h-3.5 text-amber-400" /></div><img src={event.avatar_url} alt="" className="w-7 h-7 rounded-full" /><div className="min-w-0 flex-1"><p className="text-xs"><span className="font-medium">{event.name || 'Deleted member'}</span> <span className="text-zinc-500 capitalize">{label}</span>{event.room_name && <span className="text-zinc-400"> · {event.room_name}</span>}</p></div><time className="text-[10px] text-zinc-600 whitespace-nowrap">{formatDateTime(event.created_at)}</time></div>;
...
function UserEditor({ form, setForm, selected, busy, currentUser, floors, onSave, onDelete }: any) {
  ...
  return <div className="bg-[#141418] border border-zinc-800 rounded-2xl p-5 h-fit"><h2 className="text-sm font-semibold">{selected ? 'Edit account' : 'Create account'}</h2><p className="text-xs text-zinc-600 mt-1">{selected ? `Managing @${selected.username}` : 'Add a person to this workspace'}</p><div className="space-y-4 mt-5">
    ...
    <section className="rounded-2xl border border-white/[.08] bg-zinc-950/65 p-3.5">
      <div className="flex items-center gap-3">
        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-violet-400/10 text-violet-300"><Clock3 className="h-4 w-4" /></span>
        <div className="min-w-0 flex-1"><p className="text-xs font-semibold text-zinc-300">Automatic AFK</p><p className="mt-0.5 text-[10px] leading-4 text-zinc-600">Pause tracked activity when this person is inactive.</p></div>
        <button type="button" role="switch" aria-label="Automatic AFK" aria-checked={form.afkEnabled} onClick={() => setForm({ ...form, afkEnabled: !form.afkEnabled })} className={`relative h-6 w-11 shrink-0 rounded-full border transition ${form.afkEnabled ? 'border-emerald-400/35 bg-emerald-400/20' : 'border-zinc-700 bg-zinc-900'}`}><span className={`absolute top-0.5 h-4.5 w-4.5 rounded-full transition-all ${form.afkEnabled ? 'left-[21px] bg-emerald-300' : 'left-0.5 bg-zinc-600'}`} /></button>
      </div>
      <div className={`mt-3 grid grid-cols-2 gap-2.5 transition ${form.afkEnabled ? '' : 'pointer-events-none opacity-35'}`} aria-disabled={!form.afkEnabled}>
        <label><span className="text-[10px] text-zinc-500">Mark AFK after</span><span className="mt-1 flex items-center rounded-xl border border-zinc-800 bg-black/25"><input aria-label="Mark AFK after minutes" type="number" min={1} max={240} value={form.afkAfterMinutes} onChange={(event) => { const minutes = Number(event.target.value); setForm({ ...form, afkAfterMinutes: minutes, offlineAfterMinutes: Math.max(form.offlineAfterMinutes, minutes + 1) }); }} className="min-w-0 flex-1 bg-transparent px-3 py-2 text-xs outline-none"/><span className="pr-3 text-[9px] text-zinc-600">min</span></span></label>
        <label><span className="text-[10px] text-zinc-500">Disconnect after</span><span className="mt-1 flex items-center rounded-xl border border-zinc-800 bg-black/25"><input aria-label="Disconnect after minutes" type="number" min={2} max={480} value={form.offlineAfterMinutes} onChange={(event) => setForm({ ...form, offlineAfterMinutes: Number(event.target.value) })} className="min-w-0 flex-1 bg-transparent px-3 py-2 text-xs outline-none"/><span className="pr-3 text-[9px] text-zinc-600">min</span></span></label>
      </div>
      <p className="mt-2 text-[9px] leading-4 text-zinc-600">Disconnect must occur after AFK. Turning this off keeps the member online until they leave or log out.</p>
    </section>
