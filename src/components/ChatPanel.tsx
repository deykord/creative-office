...
export const ChatPanel: React.FC<Props> = ({ currentUser, users, selectedConversationId, onSelectConversation }) => {
  ...
  return <div className="relative flex h-full min-h-0 overflow-hidden rounded-[18px] border border-white/[.07] bg-[#17171b] text-zinc-200">
    <aside className={`${selected ? 'hidden md:flex' : 'flex'} w-full md:w-[284px] shrink-0 flex-col border-r border-white/[.08] bg-[#19191d]`}>
      <div className="flex h-14 shrink-0 items-center border-b border-white/[.08] px-4">
        <button type="button" className="flex items-center gap-1 text-[15px] font-semibold">All messages <ChevronDown className="h-3.5 w-3.5 text-zinc-600" /></button>
      ...
      <div className="px-3 pt-3">
        <label className="flex h-9 items-center gap-2 rounded-lg border border-white/[.07] bg-black/20 px-3"><Search className="h-3.5 w-3.5 text-zinc-600" /><input value={conversationSearch} onChange={(event) => setConversationSearch(event.target.value)} placeholder="Search" className="w-full bg-transparent text-[11px] outline-none placeholder:text-zinc-700" /></label>
        {conversations.some((conversation) => conversation.type === 'dm') && <div className="mt-3 flex gap-4 overflow-x-auto border-b border-white/[.08] pb-3">
          {conversations.filter((conversation) => conversation.type === 'dm').slice(0, 4).map((conversation) => <button key={conversation.id} onClick={() => onSelectConversation(conversation.id)} className="w-12 shrink-0 text-center"><span className="mx-auto block h-9 w-9 overflow-hidden rounded-full bg-[#29292e] ring-1 ring-white/[.06]">{conversationAvatar(conversation) ? <img src={conversationAvatar(conversation)} alt="" className="h-full w-full object-cover" /> : <MessageCircle className="m-2.5 h-4 w-4" />}</span><span className="mt-1 block truncate text-[9px] text-zinc-400">{conversationName(conversation).split(' ')[0]}</span></button>)}
        </div>}
      </div>
      {error && <p className="mx-3 mt-2 rounded-lg bg-red-400/10 px-3 py-2 text-[10px] text-red-300">{error}</p>}
      <div className="min-h-0 flex-1 overflow-y-auto px-2 py-3">
        {sections.map((section) => <section key={section.type} className="mb-3 border-b border-white/[.07] pb-3 last:border-0">
          <div className="mb-1 flex h-7 items-center px-2 text-[11px] font-semibold text-zinc-500"><ChevronDown className="mr-2 h-3.5 w-3.5" />{section.label}{currentUser.isAdmin && section.type !== 'dm' && <button type="button" onClick={openCreate} title={`Create ${section.type}`} className="ml-auto rounded p-1 hover:bg-white/[.06] hover:text-white"><Plus className="h-3.5 w-3.5" /></button>}</div>
          {section.items.map((conversation) => <button key={conversation.id} onClick={() => onSelectConversation(conversation.id)} className={`flex min-h-11 w-full items-center gap-2 rounded-md px-2 py-1.5 text-left transition-colors ${conversation.id === selectedConversationId ? 'bg-white/[.1] text-white' : 'text-zinc-300 hover:bg-white/[.045]'}`}>
            <span className="flex h-6 w-6 shrink-0 items-center justify-center overflow-hidden rounded-full bg-[#29292e]">{conversationAvatar(conversation) ? <img src={conversationAvatar(conversation)} alt="" className="h-full w-full object-cover" /> : conversation.type === 'channel' ? <Hash className="h-3.5 w-3.5 text-zinc-500" /> : <Users className="h-3.5 w-3.5 text-zinc-500" />}</span>
            <span className="min-w-0 flex-1"><span className="block truncate text-[12px]">{conversation.isPrivate && conversation.type !== 'dm' && <Lock className="mr-1 inline h-2.5 w-2.5 text-zinc-600" />}{conversationName(conversation)}</span>{conversationPreview(conversation) && <span className="mt-0.5 flex items-center gap-1 truncate text-[8px] text-zinc-600">{conversation.unreadCount === 0 && conversation.pinnedMessage && <Pin className="h-2.5 w-2.5 shrink-0 text-amber-300" />}<span className="truncate">{conversationPreview(conversation)?.content}</span></span>}</span>
            ...
            {conversation.unreadCount > 0 && <span className="flex h-4 min-w-4 items-center justify-center rounded-full bg-[#3b82f6] px-1 text-[8px] font-bold text-white">{conversation.unreadCount}</span>}
          </button>)}
        </section>)}
      ...
      <button type="button" onClick={currentUser.isAdmin ? openCreate : () => setShowDmPicker(true)} className="flex h-12 shrink-0 items-center gap-3 border-t border-white/[.08] px-4 text-[11px] text-zinc-500 hover:bg-white/[.03] hover:text-zinc-200"><Plus className="h-4 w-4" />{currentUser.isAdmin ? 'Create group or channel' : 'New direct message'}</button>
    </aside>

    <main className={`${selected ? 'flex' : 'hidden md:flex'} min-w-0 flex-1 flex-col bg-[#1d1d21]`}>
      {selected ? <>
        <header className="flex h-14 shrink-0 items-center gap-3 border-b border-white/[.08] px-4">
          ...
          {conversationAvatar(selected) ? <img src={conversationAvatar(selected)} alt="" className="h-8 w-8 rounded-full object-cover" /> : <span className="flex h-8 w-8 items-center justify-center rounded-full bg-blue-500 text-white">{selected.type === 'channel' ? <Hash className="h-4 w-4" /> : <Users className="h-4 w-4" />}</span>}
          <div className="min-w-0"><h2 className="truncate text-[14px] font-semibold">{conversationName(selected)}</h2><p className="text-[9px] text-zinc-600">{selected.type === 'dm' ? 'Direct message' : selected.type === 'channel' ? 'Channel' : 'Group conversation'}</p></div>
        ...
        <label className="mx-4 mt-3 flex h-8 shrink-0 items-center gap-2 rounded-lg border border-white/[.06] bg-black/15 px-3"><Search className="h-3 w-3 text-zinc-700" /><input value={search} onChange={(event) => { setSearch(event.target.value); void loadMessages(selected.id, event.target.value); }} placeholder="Search this conversation" className="w-full bg-transparent text-[10px] outline-none placeholder:text-zinc-700" /></label>
        {messages.some((message) => message.pinnedAt && !message.deletedAt) && <div className="mx-4 mt-2 flex max-h-20 shrink-0 items-start gap-2 overflow-y-auto rounded-xl border border-amber-300/15 bg-amber-300/[.045] px-3 py-2"><Pin className="mt-0.5 h-3 w-3 shrink-0 text-amber-300" /><div className="min-w-0 flex-1">{messages.filter((message) => message.pinnedAt && !message.deletedAt).sort((left, right) => String(right.pinnedAt).localeCompare(String(left.pinnedAt))).map((message) => <p key={message.id} className="truncate text-[9px] text-zinc-400"><span className="font-semibold text-zinc-200">{message.sender?.name || 'Office member'}:</span> {message.content}</p>)}</div></div>}
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-3 md:px-5">
          {messages.map((message) => message.messageType === 'system' ? <div key={message.id} className="flex items-center gap-3 py-3"><span className="h-px flex-1 bg-white/[.06]" /><span className="max-w-[70%] text-center text-[9px] text-zinc-600">{message.content} · {new Date(message.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span><span className="h-px flex-1 bg-white/[.06]" /></div> : <article key={message.id} data-pinned={message.pinnedAt ? 'true' : undefined} className={`group relative flex gap-3 rounded-lg border px-1 py-2 ${message.pinnedAt ? 'border-amber-300/10 bg-amber-300/[.025]' : 'border-transparent hover:bg-white/[.018]'}`}>
            <img src={message.sender?.avatarUrl} alt="" className="mt-0.5 h-8 w-8 shrink-0 rounded-full bg-[#29292e] object-cover" />
            <div className="min-w-0 flex-1"><div className="flex items-baseline gap-2"><strong className="truncate text-[12px] font-semibold text-zinc-100">{message.sender?.name || 'Former member'}</strong><time className="text-[9px] text-zinc-600">{new Date(message.createdAt).toLocaleString([], { weekday: 'short', hour: '2-digit', minute: '2-digit' })}</time></div>{message.replyToId && <div className="my-1 border-l-2 border-blue-400/40 pl-2 text-[9px] text-zinc-600">Replying to an earlier message</div>}<p data-sticker-message={isStickerOnly(message.content) ? 'true' : undefined} className={`whitespace-pre-wrap break-words text-zinc-300 ${isStickerOnly(message.content) ? 'py-1 text-4xl leading-none drop-shadow-[0_5px_12px_rgba(0,0,0,.35)]' : 'text-[12px] leading-[1.65]'}`}>{message.content}</p>
              <div className="mt-1 flex min-h-5 items-center gap-1">{message.editedAt && <span title={`Edited ${new Date(message.editedAt).toLocaleString()}`} className="mr-1 cursor-help text-[8px] text-zinc-600">edited</span>}{message.reactions.map((reaction) => <button key={reaction.emoji} onClick={() => void api(`/api/chat/messages/${message.id}/reactions`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ emoji: reaction.emoji }) })} className="rounded-full border border-white/[.06] bg-white/[.04] px-1.5 py-0.5 text-[9px] hover:bg-white/[.08]">{reaction.emoji} {reaction.userIds.length}</button>)}
        ...
        <div className="shrink-0 px-3 pb-3 md:px-4">
          {typingNames.length > 0 && <p className="px-2 pb-1 text-[9px] text-zinc-600">{typingNames.join(', ')} typing…</p>}
          ...
          <div className={`relative border border-white/[.11] bg-[#232328] focus-within:border-white/[.2] ${replyTo ? 'rounded-b-xl' : 'rounded-xl'}`}>
            ...
            <textarea ref={composerRef} value={draft} onChange={(event) => updateTyping(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void send(); } }} rows={2} placeholder="Write a message…" className="block max-h-28 min-h-14 w-full resize-none bg-transparent px-3 pt-3 text-[12px] leading-5 outline-none placeholder:text-zinc-600" />
            <div className="flex h-10 items-center gap-0.5 px-2 text-zinc-600">
